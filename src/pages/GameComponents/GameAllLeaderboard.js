...
//     let sdata = databased.ref("games/" + this.props.game + "/leaderboard");
// sdata.on('value', (snapshot) => {
    ...
    return (
      <div>
        ...
        {this.state.count == 0 && (
          ...
          >
            <p>
              <span className="text-muted">LeaderBoard Empty</span>
            </p>
          </div>
        )}
        <div style={{ maxWidth: "100%", marginBottom: "50px" }}>
  
          {this.state.count !==5000 && Object.keys(this.state.leaderboard)
          .sort((b, a) => this.state.leaderboard[b].score - this.state.leaderboard[a].score)
            .reverse()
