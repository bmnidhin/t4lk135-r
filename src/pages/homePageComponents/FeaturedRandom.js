  ...
  render() {
    return (
      <div className='pt-5'>
        ...
        <div className='row'>
          {this.state.listen.slice(this.state.random, this.state.random + 4).map(
            (track) =>
              track.isEventPublished && (
                <div className={'col-6 col-md-3'} key={track.slug}>
                  <Link
                    to={
                      track.slug.split('/')[0] === window.location.pathname.split('/')[1]
...
export default connect(null, {nextSong})(FeaturedRandom)
